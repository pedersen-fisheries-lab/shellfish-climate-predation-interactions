### ENSURE NEWFOUNDLAND DIET DATA IS ERROR FREE ----
# Purpose: Ensure Newfoundland diet data is clean and error free
...
library(ggplot2)
                                          ...
                                          ### Download Data from temp output folder ----

# Load data
Data<- fread("data/temp/Newfoundland_Diet_Data_Cleaned.csv")
...
# stomach_id - No NAs. Should all be unique (output will read TRUE) Problem with stomach IDs
length(unique(Data$stomach_id)) == nrow(Data) # Problem with stomach IDs but confident they are unique
...
Data[predator_species=="gadus_morhua",range(shrimp_weight,na.rm=TRUE)]
...
Data[predator_species=="sebastes_mentella",range(shrimp_weight,na.rm=TRUE)]
...
Data[predator_species=="reinhardtius_hippoglossoides",range(shrimp_weight,na.rm=TRUE)]
...
Data[predator_species=="gadus_morhua",range(pandalus_sp_weight,na.rm=TRUE)]
...
Data[predator_species=="sebastes_mentella",range(pandalus_sp_weight,na.rm=TRUE)]
...
Data[predator_species=="gadus_morhua",range(pandalus_borealis_weight,na.rm=TRUE)]
...
Data[predator_species=="sebastes_mentella",range(pandalus_borealis_weight,na.rm=TRUE)]
...
Data[predator_species=="gadus_morhua",range(pandalus_montagui_weight,na.rm=TRUE)]
...
Data[predator_species=="sebastes_mentella",range(pandalus_montagui_weight,na.rm=TRUE)]
...
Data[predator_species=="reinhardtius_hippoglossoides",range(pandalus_montagui_weight,na.rm=TRUE)]
...
Data[predator_species=="gadus_morhua",range(chionoecetes_opilio_weight,na.rm=TRUE)]
...
Data[predator_species=="gadus_morhua",range(foragefish_weight,na.rm=TRUE)]
...
Data[predator_species=="sebastes_mentella",range(foragefish_weight,na.rm=TRUE)]
...
Data[predator_species=="reinhardtius_hippoglossoides",range(foragefish_weight,na.rm=TRUE)]
...
Data[predator_species=="gadus_morhua",range(other_weight,na.rm=TRUE)]
...
Data[predator_species=="sebastes_mentella",range(other_weight,na.rm=TRUE)]
...
Data[predator_species=="reinhardtius_hippoglossoides",range(other_weight,na.rm=TRUE)]
...
PlotData<-Data[!(is.na(predator_length))]
PlotData<-PlotData[!(is.na(total_weight))]
